...
export default function ChatGPTPrompt() {
  ...
  // State for Azure search
  const [azureOpen, setAzureOpen] = useState(false);
  const [azureFilter, setAzureFilter] = useState('');
  const [azureItems, setAzureItems] = useState<Array<{ id: string; title: string }>>([]);
  ...
  // 2) Open Jira search & load issues
  const openJira = () => {
    if (!projectDir) return;
    setJiraOpen(true);
    window.api
      .jiraListIssues(projectDir)
      .then((list: any[]) => setJiraIssues(list))
      .catch((e) => { alert('Failed to list Jira issues: ' + e.message); setJiraOpen(false); });
  };
  const selectJira = async (key: string) => {
    ...
    try {
      ...
      setPrompt(`Generate test cases for the following scenario from Jira ticket "${key}":\n\n${desc}`);
    } catch (err: any) {
  ...
  // 3) Open Azure search & load items
  const openAzure = () => {
    if (!projectDir) return;
    setAzureOpen(true);
    window.api
      .azureListWorkItems(projectDir)
      .then((list: any[]) => setAzureItems(list))
      .catch((e) => { alert('Failed to list Azure items: ' + e.message); setAzureOpen(false); });
  };
  const selectAzure = async (id: string) => {
    ...
    try {
      ...
      setPrompt(`Generate test cases for the following scenario from Azure work item "${id}":\n\n${desc}`);
    } catch (err: any) {
  ...
  return (
    <div className="mb-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-96 max-h-[80vh] overflow-auto p-4">
            ...
            <ul className="max-h-60 overflow-auto">
              ...
              {filteredJira.length === 0 && <li className="text-gray-500">No tickets found.</li>}
            </ul>
            <div className="mt-3 text-right">
              <button onClick={() => setJiraOpen(false)} className="px-3 py-1">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {azureOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-96 max-h-[80vh] overflow-auto p-4">
            <h3 className="text-lg font-semibold mb-2">Select Azure Work Item</h3>
            <input
              value={azureFilter}
            ...
            <ul className="max-h-60 overflow-auto">
              ...
              {filteredAzure.length === 0 && <li className="text-gray-500">No items found.</li>}
            </ul>
            <div className="mt-3 text-right">
              <button onClick={() => setAzureOpen(false)} className="px-3 py-1">Cancel</button>
            </div>
          </div>
        </div>
      )}
